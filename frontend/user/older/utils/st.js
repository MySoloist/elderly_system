...
class SpeechTranscription {
  ...
  async start(param) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        console.log('发送语音识别请求:', JSON.stringify(req))
        await this._client.start(
          ...
          (msg, isBinary) => {
            ...
            if (!isBinary) {
              ...
              console.log('解析消息:', msgObj.header.name)
              if (msgObj.header.name === "TranscriptionStarted") {
                console.log('收到 TranscriptionStarted，触发 started 事件')
                ...
                console.error('语音识别失败:', msgObj)
                this._client.shutdown()
                this._client = null
                this._event.emit("TaskFailed", str)
                this._event.emit("failed", str)
                reject(new Error(msgObj.header.status_text || '语音识别失败'))
              }
            }
          },
          //onclose
